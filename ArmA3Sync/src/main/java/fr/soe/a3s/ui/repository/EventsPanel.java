...
public class EventsPanel extends JPanel implements UIConstants {
	...
	private JButton buttonNew, buttonRemove, buttonEdit, buttonUpload, buttonSaveToDisk, buttonUploadOptions;
	...
	public EventsPanel(Facade facade, RepositoryPanel repositoryPanel) {
		...
		this.add(vertBox1, BorderLayout.CENTER);
		{
			...
			addonsSelectionPanel.setLayout(new BorderLayout());
			{
				...
				addonsSelectionPanel.setBorder(
						BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), "Addons selection"));
				addonsSelectionPanel.add(arbreScrollPane, BorderLayout.CENTER);
	...
	private void buttonUploadPerformed() {
		...
		if (eventDTOs == null) {
			JOptionPane.showMessageDialog(facade.getMainPanel(), "Nothing to upload.", "Information",
	...
	private void buttonSaveToDiskPerformed() {
		...
		if ("".equals(path) || path == null) {
			...
			JOptionPane.showMessageDialog(facade.getMainPanel(), message, "Warning", JOptionPane.WARNING_MESSAGE);
		} else if (!(new File(path)).exists()) {
			String message = "Repository main folder location: " + path + " does not exist.";
			JOptionPane.showMessageDialog(facade.getMainPanel(), message, "Warning", JOptionPane.ERROR_MESSAGE);
		} else {
			try {
				repositoryService.writeEvents(repositoryName);
				JOptionPane.showMessageDialog(facade.getMainPanel(),
						"Events informatons have been saved to the repository.", "Information",
						JOptionPane.INFORMATION_MESSAGE);
				facade.getMainPanel().updateTabs(OP_REPOSITORY_CHANGED);
